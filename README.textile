...
methods for example are now longer supported, but can be replaced with a simple
+before_filter+ call.


h2. Authors

Project initiated at "Rails-Hackathon":http://rails-hackathon.org in Madrid by:
...
translations. See the Gettext manuals for more information on this.

TODO describe how to setup the .po, .mo, and message reaper methods using Gettext.


h2. Possible Issues

We're not sure how this will perform using Rails in Multi-threaded mode as it 
depends greatly on the way Gettext works. Gettext has a tendancy to use lots of
globals so we suspect it won't work very well.
